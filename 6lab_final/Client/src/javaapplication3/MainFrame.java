...
public class MainFrame extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        calcWithServer = new javax.swing.JButton();
        ...
        calcWithServer.setBackground(new java.awt.Color(153, 255, 153));
        calcWithServer.setFont(new java.awt.Font("sansserif", 1, 12)); // NOI18N
        calcWithServer.setText("Серверное вычисление");
        calcWithServer.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                calcWithServerMouseClicked(evt);
            }
        });
            ...
            .addGroup(jPanel1Layout.createSequentialGroup()
                ...
                .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addGroup(jPanel1Layout.createSequentialGroup()
                        ...
                        .addGap(18, 18, 18)
                        .addComponent(calcWithServer, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE))
                    .addGroup(jPanel1Layout.createSequentialGroup()
    ...
    if (option == JFileChooser.APPROVE_OPTION) {
        ...
        if (!file.exists() || !file.canRead()) {
            JOptionPane.showMessageDialog(this, "Файл не существует или недоступен для чтения!", "Ошибка", JOptionPane.ERROR_MESSAGE);
            return;
        }
        ...
        try (ObjectInputStream in = new ObjectInputStream(new FileInputStream(file))) {
            ...
            int size = in.readInt(); // Чтение количества объектов
            for (int i = 0; i < size; i++) {
                RecIntegral rec = new RecIntegral();
                rec.readExternal(in); // Используем метод readExternal для каждого объекта
                data.add(rec);

                String resultDisplay = (rec.getResult() != null) ? String.valueOf(rec.getResult()) : "";
                model.addRow(new Object[]{rec.getLower(), rec.getUpper(), rec.getStep(), resultDisplay});
            }
    ...
    if (option == JFileChooser.APPROVE_OPTION) {
        ...
        try (ObjectOutputStream out = new ObjectOutputStream(new FileOutputStream(file))) {
            out.writeInt(data.size()); // Сначала записываем количество объектов
            for (RecIntegral rec : data) {
                rec.writeExternal(out); // Используем метод writeExternal для каждого объекта
            }
            JOptionPane.showMessageDialog(this, "Файл успешно сохранён!", "Успех", JOptionPane.INFORMATION_MESSAGE);
        } catch (IOException ex) {
            JOptionPane.showMessageDialog(this, "Ошибка при сохранении файла!", "Ошибка", JOptionPane.ERROR_MESSAGE);
    ...
    if (useMultiThreading) {
        ...
        for (RecIntegral rec : data) {
            ...
            for (int j = 0; j < numThreads; j++) {
                ...
                executor.execute(new IntegrationThread(rec, model, i, start, end));
            }
    ...
    private void calcWithServerMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_calcWithServerMouseClicked

    int rowIndex = jTable1.getSelectedRow();
    if (rowIndex == -1) {
        System.out.println("No row selected; action canceled.");
        return;
    }
    

    for (RecIntegral integral : data) {
        double lower = integral.getLower();
        double upper = integral.getUpper();
        double step = integral.getStep();

        
        String message = String.format("lower: %f; upper: %f; step: %f;", lower, upper, step);

       
        try {
            synchronized (ServerConnect.class) {
          
                java.io.PrintWriter out = ServerConnect.getOutputWriter();
                if (out != null) {
                    out.println(message);
            ...
            System.out.println("Error while sending data to server: " + e.getMessage());
            return;
        }
    }
    

    javax.swing.Timer timer = new javax.swing.Timer(3000, evt1 -> {
        DefaultTableModel model = (DefaultTableModel) jTable1.getModel();
        int currentRowIndex = jTable1.getSelectedRow();
        if (currentRowIndex != -1) {
            double finalSum = ServerConnect.getFinalSum();
            model.setValueAt(finalSum, currentRowIndex, 3);
            System.out.println("Updated table with final sum: " + finalSum);
        }
    });
    timer.setRepeats(false);
    timer.start();
    }//GEN-LAST:event_calcWithServerMouseClicked
    ...
    private javax.swing.JButton calcWithServer;
